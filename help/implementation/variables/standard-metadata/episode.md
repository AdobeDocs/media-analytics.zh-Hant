...
| **XDM集合欄位** | [`xdm.mediaCollection.sessionDetails.episode`](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/xdm/data-types/session-details-collection) |
...
呼叫[`sendEvent`](https://experienceleague.adobe.com/tw/en/docs/experience-platform/collection/js/commands/sendevent/overview)時，在`xdm.mediaCollection.sessionDetails`內設定`episode`：
...
呼叫`xdm.mediaCollection.sessionDetails`內有`episode`的[sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart)端點：
...
在呼叫`trackSessionStart`之前，使用`ADBMobile.media.VideoMetadataKeys.EPISODE`設定媒體物件`StandardMediaMetadata`屬性中的集數：
...
在呼叫`mediaTrackSessionStart`之前，使用`MEDIA_VideoMetadataKeyEPISODE`在媒體物件的標準中繼資料中設定集數：
