...
| **XDM集合欄位** | [`xdm.mediaCollection.sessionDetails.label`](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/xdm/data-types/session-details-collection) |
| **Audience Manager特徵** | `c_contextdata.a.media.label` |
| **必要** | 否 |
...
呼叫[`sendEvent`](https://experienceleague.adobe.com/tw/en/docs/experience-platform/collection/js/commands/sendevent/overview)時，在`xdm.mediaCollection.sessionDetails`內設定`label`：
...
呼叫`xdm.mediaCollection.sessionDetails`內有`label`的[sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart)端點：
...
在呼叫`trackSessionStart`之前，使用`ADBMobile.media.AudioMetadataKeys.LABEL`設定媒體物件之`StandardMediaMetadata`屬性中的錄製標籤：
...
在呼叫`mediaTrackSessionStart`之前，使用`MEDIA_AudioMetadataKeyLABEL`在媒體物件的標準中繼資料中設定記錄標籤：
