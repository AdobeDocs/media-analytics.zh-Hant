...
呼叫[`sendEvent`](https://experienceleague.adobe.com/tw/en/docs/experience-platform/collection/js/commands/sendevent/overview)時，在`xdm.mediaCollection.sessionDetails`內設定`feed`：
...
呼叫`xdm.mediaCollection.sessionDetails`內有`feed`的[sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart)端點：
...
在呼叫`trackSessionStart`之前，使用`ADBMobile.media.VideoMetadataKeys.FEED`在媒體物件的`StandardMediaMetadata`屬性中設定媒體摘要型別：
...
在呼叫`mediaTrackSessionStart`之前，使用`MEDIA_VideoMetadataKeyFEED`在媒體物件的標準中繼資料中設定媒體摘要型別：
