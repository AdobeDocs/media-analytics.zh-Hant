...
* **`_data`格式：**&#x200B;欄位放在`_data.__adobe.analytics.contextData`下。 欄位名稱不需要底線前置詞（例如，`debugFlag`）。
...
當您在Adobe Analytics中需要&#x200B;**不應**&#x200B;儲存於AEP資料集中的中繼資料時，請使用`_data`物件。 範例包括暫存旗標、偵錯變數或Analytics專用的處理提示。
