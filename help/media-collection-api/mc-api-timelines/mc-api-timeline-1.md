...
這個呼叫代表&#x200B;_使用者有意願播放_&#x200B;視訊。<br/><br/>它會傳回會話ID( `{sid}`)給用戶端，用來識別作業中所有後續追蹤呼叫。 播放器狀態尚未進入「正在播放」，而是「正在開始」。<br/><br/>[必要的作業 ](/help/media-collection-api/mc-api-ref/mc-api-sessions-req.md) 參數必須包含在請 `params` 求主體的地圖中。 <br/><br/>在後端，這個呼叫會產生 Adobe Analytics 起始呼叫。
...
將播放狀態改變為「正在播放」。**在`play`呼叫`pauseStart`後，會向後端輸入「繼續」呼叫，** 因此不需要事 `resume` 件。
