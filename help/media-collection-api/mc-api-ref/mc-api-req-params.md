...
seo-title: 要求參數
title: 要求參數
uuid: f83 e9 ef1-803d-4152-a6 c7-aca325036 b9
...
| `analytics.visitorId` | N | `sessionStart` | Adobe訪客ID是可用於多個Adobe應用程式的自訂ID。The Heartbeat `visitorId` equals the Analytics `VID.` |
...
| `visitor.marketingCloudUserId` | N | `sessionStart` | 這是Experience Cloud使用者ID(ECID)。在大部分情況下，這是您用來識別使用者的ID。The Heartbeat `marketingCloudUserId` equals the `MID` in Adobe Analytics. 在技術上不需要，這個參數是存取Experience Cloud應用程式系列時的必要參數。 |
...
| `media.resume` | N | `sessionStart` | 指出使用者是否正在繼續先前的工作階段(相對於起始新作業) |
...
>[!NOTE]
>
>媒體分析(MA)已與Experience Cloud應用程式系列(Adobe Analytics、Audience Manager、Target等)整合。若要存取這些應用程式，您需要 Experience Cloud ID。_在大多數情況下，您應該使用ECID來識別使用者。_

### appInstallationId

* **如果您&#x200B;*未*傳遞`appInstallationId`值-** MA後端將不再產生MCID，而是依賴Adobe Analytics執行此動作。Adobe 建議您盡可能傳送 MCID 或 `appInstallationId` (連同目前仍為必要項目的 `marketingCloudOrgId`)，讓媒體收集 API 在每次呼叫時產生 MCID 並予以傳送。

* ***如果您是*通過`appInstallationId`值，** 則MA後端 *可* 產生MCID，如果您傳遞值 `appInstallationId` 和(必要) `marketingCloudOrgId` 參數。如果您自行傳遞 `appInstallationId`，必須將其值保留在用戶端上。該值必須供裝置上的應用程式專用，而且只要您未重新安裝應用程式，該值就必須存在。
...
此參數指出當Adobe Analytics將客戶資料傳送至Audience Manager時，會點擊哪些Adobe Audience Manager(AAM) Edge。如果您未傳遞這個參數，Adobe 會將其硬式編碼為 1。當使用者傾向於在偏遠位置使用裝置 (如美國東部、美國西部、歐洲、亞洲) 時，這個參數尤其重要。否則，使用者資料將散佈到多個 AAM Edge。
