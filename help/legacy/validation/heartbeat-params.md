...
| l:asset:持續時間/ l:asset:length <br/>（從長度重新命名） | VideoInfo | (必要)<br/><br/>主要資產的影片資產長度。 |
| s:asset:發佈者 | MediaHeartbeatConfig | (必要)<br/><br/>資產的發佈者。 |
...
| s:user:援助 | Experience Cloud 組織 | (選用)<br/><br/>使用者的 Analytics 訪客 ID 值。 |
...
| s:sp:頻道 | MediaHeartbeatConfig | (選用)<br/><br/>使用者觀看內容的頻道。若為行動應用程式，則為應用程式名稱。若為網站，則為網域名稱。 |
...
| 名稱 | 資料來源 | 說明 |
| ---  | --- | --- |
| s:event:來源 | Media SDK | (必要)<br/><br/>錯誤的來源，可能是播放器內部或應用程式層級。 |
...
| s:asset:解析程式 | AdBreakInfo | (必要)<br/><br/>廣告解析程式。 |
