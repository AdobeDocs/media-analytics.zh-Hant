...
To set-up video reporting for a Milestone implementation, go to **[!UICONTROL Admin &gt; Report Suite Manager].** 選取報表套裝，然後選擇「視訊 **[!UICONTROL 管理&gt;視訊報表」]:**
...
| 視訊度量 | 變數類型 | 說明 |
| --- | --- | --- |
| 內容 | eVar預 <br/>設有效期：瀏覽 | (必要) 依實作指示收集視訊名稱。 |
| 內容類型 | eVar <br/>Default expiration: Page view | 收集訪客所檢視內容類型的相關資料。視訊測量所傳送的點擊會指派內容類 `video.` 型。此變 <br/>數不需專門保留供視訊追蹤使用。 使用相同變數擁有其他內容報告內容類型可讓您分析不同內容類型的訪客分佈。 舉例來說，使用了這個變數，您就可以利用像是 `article` 或 `product page` 等值來標記其他內容類型。<br/>從視訊測量觀點，「內容類型」**&#x200B;可讓您識別視訊訪客，並因此計算視訊轉換率。 |
...
| 變數    | 說明 |
| --- | --- |
| `Media.trackUsingContextData` | **語法:** <br/><br/> `s.Media.trackUsingContextData = true;`<br/>此選項可啟用整合式視訊追蹤。當設為true時，媒體模組會產生媒體追蹤的上下文資料，而非舊版 `pev3`。 <br/>使用 `Media.contextDataMapping` 來映射上下文資料至選取的 eVar 和事件。<br/>預設值： `false` |
| `Media.contextDataMapping` | **語法:** <br/><br/> `s.Media.contextDataMapping = {`<br/>      `"a.media.name":"eVar2, prop2",` <br/>     `"a.media.segment":"eVar3",` <br/>     `"a.contentType":"eVar1",` <br/>     `"a.media.timePlayed":"event3",` <br/>     `"a.media.view":"event1",` <br/>     `"a.media.segmentView":"event2",` <br/>     `"a.media.complete":"event7",` <br/>     `"a.media.milestones":{` <br/>         `25:"event4",` <br/>         `50:"event5",` <br/>         `75:"event6"` <br/>     ` }` <br/> `};`<br/><br/>一個物件，定義您要用於視訊測量的變數映射至 eVar 和事件。物件必須映射下列欄位: <br/><br/> **a.media.name:** (必要) 以視訊名稱填入變數。提供您選取要儲存視訊名稱的 eVar，以及要用於視訊路徑的「自訂分析視訊」變數 (`s.prop`)。Provide the values in a comma-separated list. <br/><br/> **a.media.segment:** (可選) 您要儲存媒體區段名稱的 eVar。a.contentType: (可選) 您要儲存視訊值的 eVar，其包含已啟用的造訪和訪客追蹤，用以產生視訊造訪和訪客報表。您選取的變數似乎已用來儲存文章投影片放映或產品頁面之類的資料 <br/><br/> **** a.media.view:（必要）您要計算媒體檢視次數的事件。 <br/><br/> **** a.media.segmentView:（選用）您要計算區段檢視的事件。 <br/><br/> **** a.media.complete:（可選）您要計算完整檢視的事件。 <br/><br/> **** a.media.timePlayed:（選擇性，強烈建議）您要儲存播放視訊秒數的數值事件。 <br/><br/> **a.media.milestones:** (可選) 將 s.Media.trackMilestones 里程碑與計數器事件對應的物件。Media.segmentByMilestones should be set to true if you define milestones. <br/><br/> **廣告追蹤** ：若要追蹤廣告，可使用下列上下文資料變數： <br/> **a.media.ad.name:** (必要) 以廣告名稱填入變數。提供您選取要儲存廣告名稱的 eVar，以及要用於路徑的「自訂分析視訊」變數 (`s.prop`)。Provide the values in a comma-separated list. <br/><br/> **** a.media.ad.pod:播放廣告的主要內容中的位置。 <br/><br/> **** a.media.ad.podPosition:播放廣告的Pod內位置。 <br/><br/> **** a.media.ad.CPM:套用至此播放的CPM或加密CPM（前置詞為"~"）。 <br/><br/> **a.media.ad.view:** 與 的作用相同。`a.media.view`<br/><br/> **** a.media.ad.clicked:計算廣告的點按次數(呼`Media.click` 叫) <br/><br/> **a.media.ad.timePlayed:** 與 的作用相同。`a.media.timePlayed`<br/><br/> **** a.media.ad.complete:與a.media.ad.se區段的作 `a.media.complete` 用相同：與 `a.media.segment`<br/><br/> **a.media.ad.segmentView:** 與 的作用相同。`a.media.segmentView`<br/><br/> **a.media.ad.milestones:** 與 的作用相同。`a.media.milestones`<br/><br/> **a.media.ad.offsetMilestones:** 與 的作用相同。`a.media.offsetMilestones` |
...
|  變數    | 說明 |
| --- | --- |
| `Media.autoTrack` | **語法:** <br/><br/> `s.Media.autoTrack = true`<br/><br/>啟用對支援的播放器的自動追蹤。支援的播放器如下所示: <ul> <li> Open Source Media Framework (OSMF) </li> <li> FLVPlayback (透過 Flash Professional 中的匯入視訊精靈建立的視訊播放器) </li> <li> Silverlight </li> <li> MediaDisplay </li> <li> MediaPlayback </li> <li> Brightcove API 版本 2 與 3 (請參閱 [Brightcove](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/video/video_other_players.html)) </li> <li> 使用 JavaScript 的 Windows Media Player、Quicktime 或 Real Player </li> </ul> <br/><br/>如果您未使用上述其中一個播放器，則可使用它來 `Media.open` 追 `Media.play``Media.stop` 蹤播 `Media.close` 放器事件。 |
...
| `Media.monitor` | **語法:** <br/><br/> `s.Media.monitor(s, media)` <br/><br/> **Silverlight 語法:** <br/><br/> `s.Media.monitor =` <br/>Silverlight `new AppMeasurement_Media_Monitor(myMediaMonitor);`<br/><br/>應用程式媒體監視器會實作Objective-C委派設計模式。 類 `myMediaMonitor` 方法採用 `s` 和 `media` 參數。 <br/><br/>使用此方法來傳送其他視訊測量。您可以設定其他變數 (Prop、eVar、事件)，並在播放視訊時根據視訊的目前狀態，使用 `Media.track` 傳送它們。<br/><br/>請參 [閱使用Media.monitor測量其他量度。](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/video/video_mediamonitor.html) <br/><br/>此方法會採用下列參數: <br/><br/>  **** s:例 `AppMeasurement` 項(或JavaScript `s` 物件)。 <br/><br/> **** 媒體：具有提供視頻狀態的成員的對象。 這些成員包括:  <ul><li> `media.name:` 視訊的名稱。 This must match the name used in `Media.open`; </li><li> `media.length:` 視訊在呼叫時的秒數長度 `Media.open`; </li><li> `media.playerName:` 呼叫中指定的媒體播放器名稱 `Media.open`; </li><li> `media.openTime:` NSDate物件，包含呼叫時間 `Media.open` 的相關資料； </li><li> `media.offset:` 視訊中目前的偏移（以秒為單位）（視訊中的實際點）。 偏移從零開始（視頻的第一秒是第二個0）; </li><li> `media.percent:` 根據視頻長度和當前偏移量，當前已播放視頻的百分比；  </li><li> `media.timePlayed:` 目前播放的秒數總計；  </li><li> `media.eventFirstTime:` 指出這是否是第一次針對此視訊呼叫此媒體事件； </li><li> `media.mediaEvent:` 包含導致監視器呼叫的事件名稱的字串。 </li></ul> |
|  | `media.mediaEvent` 事件： <ul><li> `OPEN:` 首次通過或呼叫觀 `Media.autoTrack` 察播放時 `Media.play`; </li><li> `CLOSE:` 當播放結束時，視訊會透過或 `Media.autoTrack` 呼叫結束 `Media.close`;</li><li> `PLAY:` 當播放在暫停或拖曳通過或第二次呼叫 `Media.autoTrack` 後繼續播放時 `Media.play`;</li><li> `STOP:` 當播放因拖曳開始或呼叫而停止 `Media.autoTrack` 時 `Media.stop`;</li><li> `MONITOR:` 當我們的自動監控功能在視訊播放時（每秒）檢查視訊的狀態；</li><li> `SECONDS:` 以變數所定義的第二個區間 `Media.trackSeconds` 為準；</li><li> `MILESTONE:` 在變數定義的里程碑 `Media.trackMilestones` 處； </li></ul> |
...
* **** 暫停：打 `Media.stop`電話。 例如，使用者若在 15 秒後暫停視訊，則呼叫 `s.Media.stop("Video1", 15)`
* **緩衝:** 當視訊緩衝時呼叫 `Media.stop`。Call `Media.play` when playback resumes.
* **** 繼續：打 `Media.play`電話。 例如，使用者若在起始播放視訊 15 秒後繼續播放視訊，則呼叫 `s.Media.play("Video1", 15)`。
* **Scrub (滑桿):** 當使用者拖曳視訊滑桿時，呼叫 `Media.stop`。當使用者放開視訊滑桿時，呼叫 `Media.play`。
* **** 結束：那 `Media.stop`就打 `Media.close`。 For example, at the end of a 100-second video, call `s.Media.stop("Video1", 100)`, then `s.Media.close("Video1")`.
