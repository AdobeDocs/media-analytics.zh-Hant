...
媒體後端會針對任何涵蓋區段的關閉呼叫設定`mediaReporting.sessionDetails.hasSegmentView = true`，其中至少已收到主要內容的一個`media.play`事件。 量度會在關閉呼叫時回報。 在Media Edge API路徑上，區段檢視會在與內容開始的相同條件下引發。 這兩者都需要主要內容上的`media.play`。
